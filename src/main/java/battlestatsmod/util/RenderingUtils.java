...
public class RenderingUtils {
    ...
    // TODO: verify if the .cpy() copies are actually required (i doubt it, but
    // original codebase does it)
    private static Color identifyColor(String word) {
        ...
        if (word.length() > 1 && word.charAt(0) == '#') {
            switch (word.charAt(1)) {
                ...
                case 'g':
                    return ColorHelper.GRAY_COLOR.cpy();
                case 'r':
                    return ColorHelper.RED_COLOR.cpy();
                case 'd':
                    return ColorHelper.DEBUFF_COLOR.cpy();
                case 'u':
                    return ColorHelper.BUFF_COLOR.cpy();
                case 'b':
                    return ColorHelper.BLOCK_COLOR.cpy();
                case 's':
                    return ColorHelper.SPECIAL_COLOR.cpy();
                case 'm':
                    return ColorHelper.rainbow();
                default:
                    return null;
                // NOTE: old version returned white; this returns null to let the modified
                // smartText function handle it
    ...
    // Modified version of the base code to handle my own special font colors and
    // rendering
